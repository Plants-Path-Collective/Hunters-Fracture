...
public class TimelineTurnTest : MonoBehaviour
{
    ...
    public List<Unit> _party = new List<Unit>();
    ...
    private void OnHeal()
    {
        ...
        var actor = _battle.CurrentActor;
        var mc = actor as Unit;
        if (mc != null && mc.SP < 5)
        {
            Log($"{actor.Name} doesn't have enough SP!");
    ...
    private void RefreshStatus()
    {
        for (int i = 0; i < _party.Count; i++)
        {
            ...
            if (i < partyMPBars.Length && partyMPBars[i] != null)
                partyMPBars[i].value = c.MaxSP > 0 ? (float)c.SP / c.MaxSP : 0;
            ...
            if (i < partyMPTexts.Length && partyMPTexts[i] != null)
                partyMPTexts[i].text = $"SP {c.SP}/{c.MaxSP}";
    ...
    private void RefreshCommandButtons()
    {
        ...
        btnHeal.interactable = isPriest && mc.SP >= 5;
